...
if [[ $org_name == *['!'@#\$%^\&*()_+?~/=]* || $org_name =~ "<" || $org_name =~ ">" || $org_name =~ "|" || $org_name =~ "}" $org_name =~ "{" || $org_name =~ "}" || $org_name =~ "[" || $org_name =~ "]" || $org_name =~ [[:upper:]] || $firstChar == *['!'@#\$%^\&*()_+?=]* || $lastChar == *['!'@#\$%^\&*()_+?=]* || $org_name = *[[:space:]]* || $firstChar = *[[:space:]]* || $lastChar = *[[:space:]]* || $len -lt 3 || $len -gt 20 ]]
  then
